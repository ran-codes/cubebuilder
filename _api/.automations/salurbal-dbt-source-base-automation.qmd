---
title: "salurbal_etl_dbt__v_1_2__base_automation"
subtitle: "This notebook takes loaded source data and metadata parquet files (schema v2+) and initializes then as source, base and core administrative models within DBT."
author: 
  - name: Ran Li (Maintainer)
  - name: Usama Bilal (Context Partner)
date: last-modified
format: 
  html:
    toc: true
...
setwd(here::here('code/dbt/.automations'))
pacman::p_load(tidyverse,yaml, arrow, assertr, reactable, glue, cli)
...
local_context = lst(
  df_inventory_schema_v2 = list.files(
    '//files.drexel.edu/colleges/SOPH/Shared/UHC/Projects/Wellcome_Trust/Data Methods Core/Dashboards/dbt/salurbal-dbt-server/sources', 
...
We have our basic global setup and then we operationlized an inveotry of production (schema v2+) cube assets that were staged as sources. Let preview that first
...
df_salurbal_source_input = local_context$df_production_inventory  %>% 
  ...
  pivot_longer(cols = -dataset_instance,
              values_to = 'path') %>% 
  mutate(type = str_remove_all(name, "path_dbt_source_"),
...
```{r}
int__metadata_model_id = glue("cte__schema_v2_metadata_cube")
sql_endpoint = glue("../models/core/0_precensor/{int__metadata_model_id}.sql")
```

Now lest opertaionlize the information on the abse models we want to compile. Lets first operatinalize our input which is a list of base models for each cube type - here we filter for all metadata cubes.
...
## Prep
int__data_model_id = glue("cte__schema_v2_data_cube")
